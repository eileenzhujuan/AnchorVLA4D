...
class SinusoidalPositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        half_dim = self.dim // 2
        exponent = torch.arange(half_dim, device=device) * -math.log(10000) / (
            half_dim - 1)  # shape: (D/2,)
        emb = torch.exp(exponent)  # shape: (D/2,)
        emb = x[:, None] * emb[
            None, :]  # shape: (batch_size, 1) * (1, D/2) -> (batch_size, D/2)
        emb = torch.cat((emb.sin(), emb.cos()),
                        dim=-1)  # shape: (batch_size, D)
        return emb
...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.model = MLPResNet(num_blocks=2,
                               input_dim=input_dim * self.action_dim,
...
class DiffusionActionHead(nn.Module):
    ...
    def __init__(
        ...
        self.action_dim = action_dim
        self.action_chunk = action_chunk
        self.action_projector = NoisyActionProjector(input_dim)
        self.noise_predictor = NoisePredictionModel(
            transformer_hidden_dim=input_dim * self.action_dim,
            hidden_dim=hidden_dim,
    ...
    def sample_noisy_actions(self, ground_truth_actions, sample_method='pi0'):
        ...
        noise_shape = (batch_size, self.action_chunk, self.action_dim)
        if sample_method == 'pi0':  # pi0.5
            noise = torch.normal(mean=0.0,
                                 std=1.0,
                                 size=noise_shape,
                                 dtype=torch.float,
                                 device=device).to(dtype=dtype)
            time_beta = self.sample_beta(1.5, 1.0, batch_size, device, torch.float)
            timesteps = (time_beta * 0.999 + 0.001).to(dtype=dtype,
                                                       device=device)
            time_expanded = timesteps[:, None, None]
            noisy_actions = time_expanded * noise + (
                1 - time_expanded) * ground_truth_actions

        else:  # openvla-oft, wall-x
            noise = torch.randn(size=noise_shape, device=device,
                                dtype=dtype)  # (B, chunk_len, action_dim)
            ...
            noisy_actions = self.noise_scheduler.add_noise(
                ground_truth_actions, noise,
                timesteps)  # (B, chunk_len, action_dim)

        # Get diffusion timestep embeddings as well
        diffusion_timestep_embeddings = self.time_encoder(timesteps).to(
            noisy_actions.dtype).to(noisy_actions.device)  # (B, llm_dim)
        diffusion_timestep_embeddings = diffusion_timestep_embeddings.unsqueeze(
            1)  # (B, 1, llm_dim)

        delta_noise = noisy_actions - ground_truth_actions
        return_dict = dict(
            noise=delta_noise if sample_method == 'pi0' else noise,
    ...
    def predict_noise(self, actions_hidden_states):
        ...
        batch_size = actions_hidden_states.shape[0]
        rearranged_actions_hidden_states = actions_hidden_states.reshape(
            batch_size, self.action_chunk,
            -1)  # (batch_size, chunk_len, action_dim * hidden_dim)
