...
class ScaleDP(PreTrainedModel):
    ...
    def __init__(
        ...
        self.state_as_cond = config.state_dim > 0 and config.state_embed_dim > 0
        self.global_nd_pool = nn.AdaptiveAvgPool1d(config.adaptive_pool_dim)
        vlm_input_dim = config.cond_dim * config.adaptive_pool_dim
        self.norm_after_pool = nn.LayerNorm(vlm_input_dim)
        if self.state_as_cond:
            self.state_linear = nn.Linear(config.state_dim, config.state_embed_dim)
        self.combine = nn.Sequential(
            nn.Linear(vlm_input_dim + config.state_embed_dim, 1024),
            nn.ReLU(),
            nn.Linear(1024, 1024),
            nn.ReLU(),
        ...
        self.injection_layers = {(config.depth//3 - 1): 0, (config.depth//3*2 -1): 1, (config.depth-1): 2}
        if config.spatial_attn:
            self.cross_attention = nn.ModuleList([CrossAttention(d_model=config.n_emb, num_heads=config.num_heads) for _ in self.injection_layers])
        else:
    ...
    def forward(self, actions, hidden_states, states, spatial_features, is_pad):
        """
        ...
        if actions is not None:  # training time
            ...
            if states is not None:
                states = states.view(B, -1).repeat(num_noise_samples,  1)
            ...
            with torch.no_grad():
                ...
                alphas_cumprod = self.noise_scheduler.alphas_cumprod.to(actions.device)
                t = timesteps  # [B * num_noise_samples]
                sqrt_alpha_bar = torch.sqrt(alphas_cumprod[t]).view(-1, 1, 1)
                sqrt_one_minus_alpha_bar = torch.sqrt(1 - alphas_cumprod[t]).view(-1, 1, 1)
                # x0 prediction
                action_pred = (
                    noisy_actions - sqrt_one_minus_alpha_bar * noise_pred
                ) / sqrt_alpha_bar
    ...
    def model_forward(self, x, t, global_cond, states, spatial_features):
        """
        ...
        """
        # (N, seq_len, D) -> (N, D, adaptive_pool_dim)
        global_cond = self.global_nd_pool(global_cond.transpose(1, 2))
        # -> (N, adaptive_pool_dim * D)
        batch_size = global_cond.shape[0]
        global_cond = global_cond.transpose(1, 2).view(batch_size, -1)
        global_cond = self.norm_after_pool(global_cond)
        # -> (N, adaptive_pool_dim * D + state_embed_dim)
        if self.state_as_cond and states is not None:
            state_emb = self.state_linear(states)
            global_cond = torch.cat([global_cond, state_emb], dim=-1)
        ...
        if spatial_features is not None:
            spatial_features_list = spatial_features.transpose(0, 1).chunk(len(self.injection_layers), dim=1)
        for idx, block in enumerate(self.blocks):
            if spatial_features is not None:
                if idx in self.injection_layers.keys():
                    cross_attn_idx = self.injection_layers[idx]
                    x = x + self.cross_attention[cross_attn_idx](x, spatial_features_list[cross_attn_idx])
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        strict = skip_key(['combine.0.weight', 'pos_embed',
                           'x_embedder.weight',
                           'norm_after_pool.weight', 'norm_after_pool.bias',
                           'final_layer.linear.weight', 'final_layer.linear.bias'
                          ], [
                           self.combine[0].weight.shape, self.pos_embed.shape,
                           self.x_embedder.weight.shape,
                           self.norm_after_pool.weight.shape, self.norm_after_pool.bias.shape,
                           self.final_layer.linear.weight.shape, self.final_layer.linear.bias.shape
                          ])
        if hasattr(self, "state_linear"):
            strict = skip_key([
                           'state_linear.weight', 'state_linear.bias',
                          ], [
                           self.state_linear.weight.shape, self.state_linear.bias.shape,
                           ]) and strict
