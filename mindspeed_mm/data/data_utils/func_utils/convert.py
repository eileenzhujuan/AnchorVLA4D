...
class DatasetConverter:
    ...
    def _find_medias(self, medias: Union["MediaType", List["MediaType"], None]) -> Optional[List["MediaType"]]:
        ...
        for i, media in enumerate(medias):
            if self.data_args.dataset_dir.startswith("obs://") or os.path.isfile(os.path.join(self.data_args.dataset_dir, media)):
...
class SharegptDatasetConverter(DatasetConverter):
    def __call__(self, example: Dict[str, Any]) -> Dict[str, Any]:
        ...
        output = {
            ...
            "_videos": self._find_medias(example[self.dataset_attr.videos]) if self.dataset_attr.videos else None,
            "_audios": self._find_medias(example[self.dataset_attr.audios]) if self.dataset_attr.audios else None,
            "_states": example[self.dataset_attr.states] if self.dataset_attr.states else None,
        }
        return output
...
class DatasetAttr:
    ...
    states: Optional[List[float]] = None
    # sharegpt columns
...
class DataArguments:
    ...
    dataset_use: str = field(
        default="",
        metadata={"help": "datasetname and path defined in mindspeed_mm/data/datasets/data_qwen_packed.py,currently support llava_inhstruct_150k,llava_inhstruct_150k_pack,cambrian_737k"},
    )
    pack: bool = field(
        default=False,
        metadata={"help": "Enable data packing."},
    )
...
class SupervisedDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        ...
        actions = eval(response[0]['content'])
        encoded_pairs = self.template.encode_multiturn(self.tokenizer, self.action_tokenizer, messages, system)
        ...
        return input_ids, labels, actions, states, images
    ...
    def preprocess_dataset(self, examples: Dict[str, List[Any]]) -> Dict[str, List[Any]]:
        ...
        for i in range(len(examples["_prompt"])):
            ...
            input_ids, labels, actions, states, image_names = self._encode_data_example(
            ...
            model_inputs["states"].append(states)
            model_inputs["actions"].append(actions)
            model_inputs["image_names"].append(image_names)
