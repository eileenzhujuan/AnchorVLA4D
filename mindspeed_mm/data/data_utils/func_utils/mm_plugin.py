...
from ..transform_pipeline import TransformMapping
...
try:
    ...
    obs_client = ObsClient(access_key_id=os.getenv("AccessKeyID"),
    ...
    def _regularize_images(self, images: List["ImageInput"], **kwargs) -> Dict[str, List["ImageObject"]]:
        ...
        for image in images:
            if isinstance(image, (str, BinaryIO)):
                if isinstance(image, str) and image.startswith("obs://"):
                    if obs_client is not None:
                        resp = obs_client.getObject(
                            bucketName=bucket_name,
                            objectKey=image.replace(f"obs://{bucket_name}/", ""),
                            loadStreamInMemory=True)
                        # 返回码为2xx时，接口调用成功，否则接口调用失败
                        if resp.status < 300:
                            image = BytesIO(resp.body.buffer)
                        else:
                            raise Exception(f"Trying to get image from {image}, but failed")
                try:
                    if isinstance(image, str):
                        with open(image, 'rb') as image_f:
                            image_read = image_f.read()
                    else:
                        image_read = image
                    image = Image.frombytes("RGB", (224, 224), image_read)
                except:
...
class BasePlugin(MMPluginMixin):
    ...
    def get_mm_inputs(
            ...
            processor: Optional["MMProcessor"],
            image_aug: Optional[List[dict]] = None,
        ...
        """
        self._validate_input(processor, images, videos, audios)
        return self._get_mm_inputs(images, videos, audios, processor, image_aug)


@dataclass
class Qwen2VLPlugin(BasePlugin):
    _transforms_compose = None

    def _image_transform(self, image, aug_transforms):
        if self._transforms_compose is None:
            pipeline = []
            if aug_transforms is not None:
                for _trans in aug_transforms:
                    trans_type = _trans.get("trans_type", "")
                    trans_info = TransformMapping(is_video=False, trans_type=trans_type, ).get_trans_func()
                    pipeline.append(trans_info)
            self._transforms_compose = transforms.Compose(pipeline)
        return self._transforms_compose(image)

    @override
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        image_aug_transforms = kwargs.pop("image_aug")
        ...
        if image.height / image.width > 200:
            ...
            image = image.resize((width, height))
        image = self._image_transform(image, image_aug_transforms)
        return image
    ...
    def process_messages(
        ...
        if self.expand_mm_tokens:
            image_aug = getattr(processor, "image_aug")
            mm_inputs = self._get_mm_inputs(images, videos, audios, processor, image_aug)
