...
class DataCollatorForQwen2vl:
    def __init__(self, ignore_pad_token_for_loss: bool, dataset_param=None, **kwargs):
        ...
        image_aug_args = dataset_param.preprocess_parameters.to_dict().get('image_aug', None)
    ...
    def __call__(self, *args, **kwargs):
        # Extract metadata that shouldn't be tensorized
        metadata = [f.pop("image_names", None) for f in args[0]]
        
        # Use the standard collator for the remaining numeric data
        batch = self.data_collator(*args, **kwargs)
        
        # Add the metadata back as a simple list
        batch["image_names"] = metadata
        return batch
