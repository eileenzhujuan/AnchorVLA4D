...
def initialize_sequence_parallel_group(sequence_parallel_size):
    ...
    if not world_size % sequence_parallel_size == 0:
        raise AssertionError(
            f"world_size must be divisible by sequence_parallel_size, but got world_size: {world_size}, sequence_parallel_size: {sequence_parallel_size}")
