...
from mindspeed_mm.models.anchor_vla import AnchorVLA
...
def model_provider(pre_process=True, post_process=True, modules=None):
    ...
    args = get_args()
    print_rank_0("building VLAModel ...")
    vlm_config = deepcopy(args.mm.model)
    action_config = vlm_config.action_head.to_dict()
    ...
    model = AnchorVLA(vlm_config, action_config)
...
def _apply_freezing(model, vlm_config):
    ...
    has_info_sharing = hasattr(vlm_config.action_head, 'info_sharing') and vlm_config.action_head.info_sharing is not None
    freeze_info_sharing = has_info_sharing and getattr(vlm_config.action_head.info_sharing, 'freeze', False)

    model.freeze(
        freeze_text_decoder=freeze_text_decoder,
        freeze_image_encoder=freeze_image_encoder,
        freeze_image_projection=freeze_image_projection,
        freeze_info_sharing=freeze_info_sharing
    )
...
def get_batch(data_iterator):
    """Generate a batch."""
    # return torch.load('batch_data.pt')
    ...
    if 'image_names' in batch.keys():
        batch.pop('image_names')
    if has_video:
        batch['pixel_values'] = batch.pop('pixel_values_videos')
        batch['image_grid_thw'] = batch.pop('video_grid_thw')
    has_actions = 'actions' in batch
    if has_actions:
        batch['ground_truth_actions'] = batch.pop('actions')
    return batch
...
def loss_func(output_tensor):
    ...
    if "action_loss" in output_tensor.keys():
        loss_dir["action_loss"] = output_tensor["action_loss"]
