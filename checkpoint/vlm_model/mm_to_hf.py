...
def load_from_mm(load_dir: Path,
    ...
    for tp_rank in range(tp_size):
        ...
        for pp_rank in range(len(vit_pp_list)):
            ...
            pt_state_dict = torch.load(pt_path, map_location='cpu', weights_only=False)['model']
            pp_state_dict['vlm'].update({
                rename_pp_parameter(param, vit_pp_list, llm_pp_list, audio_pp_list, pp_rank): tensor
                 for param, tensor in pt_state_dict['vlm'].items()
                 if tensor is not None})
            if 'vit_memory' in pt_state_dict.keys():
                pp_state_dict['vit_memory'] = pt_state_dict['vit_memory']
            if 'info_sharing' in pt_state_dict.keys():
                pp_state_dict['info_sharing'] = pt_state_dict['info_sharing']
            pp_state_dict['action_head'].update(pt_state_dict['action_head'])
...
def convert_mm_to_hf(convert_config: ConvertHFConfig,
    ...
    if convert_config.contain_action_head:
        vlm_state, action_state = state_dict['vlm'], state_dict['action_head']
        if 'vit_memory' in state_dict.keys():
            mid_state = state_dict['vit_memory']
        else:
            mid_state = None
        if 'info_sharing' in state_dict.keys():
            info_sharing = state_dict['info_sharing']
        else:
            info_sharing = None
        state_dict = vlm_state
    cross_state = {key:value.clone() for key, value in state_dict.items() if key.startswith('image_cross_attn')}
    for op in ops:
    ...
    if convert_config.contain_action_head:
        action_state_save = {k: v.clone() for k, v in action_state.items()}
        torch.save(action_state_save, os.path.join(convert_config.save_hf_dir, 'action_head.pt'))
        if mid_state is not None:
            mid_state_save = {k: v.clone() for k, v in mid_state.items()}
            torch.save(mid_state_save, os.path.join(convert_config.save_hf_dir, 'vit_memory.pt'))
        if info_sharing is not None:
            info_sharing_save = {k: v.clone() for k, v in info_sharing.items()}
            torch.save(info_sharing_save, os.path.join(convert_config.save_hf_dir, 'info_sharing.pt'))
        if cross_state is not None:
            torch.save(cross_state, os.path.join(convert_config.save_hf_dir, 'cross_state.pt'))
